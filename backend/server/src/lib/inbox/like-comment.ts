/**
 * Like a comment on the originating platform (heart on inbound comments).
 */
...
async function likeFacebook(input: LikeCommentInput): Promise<LikeCommentResult> {
  const url = `https://graph.facebook.com/v21.0/${encodeURIComponent(input.commentId)}/likes`;
  ...
  if (!res.ok) {
    return fail(data.error?.message ?? "Facebook like failed");
  }
  return { ok: true };
}

async function likeInstagram(input: LikeCommentInput): Promise<LikeCommentResult> {
  const url = `https://graph.instagram.com/v21.0/${encodeURIComponent(input.commentId)}/likes`;
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({ access_token: input.accessToken }),
  ...
  if (!res.ok) {
    return fail(data.error?.message ?? "Instagram like failed");
  }
  return { ok: true };
}
...
async function likeTwitter(input: LikeCommentInput): Promise<LikeCommentResult> {
  ...
  try {
    ...
    return fail(e instanceof Error ? e.message : "X like failed");
  }
}
...
async function likeBluesky(input: LikeCommentInput): Promise<LikeCommentResult> {
  ...
  if (!threadRes.ok || !post?.uri || !post.cid) {
    return fail("Could not load the Bluesky comment to like.");
  }
...
async function likeThreads(input: LikeCommentInput): Promise<LikeCommentResult> {
  const url = `https://graph.threads.net/v1.0/${encodeURIComponent(input.commentId)}/likes`;
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: new URLSearchParams({ access_token: input.accessToken }),
  ...
  if (!res.ok) {
    return fail(data.error?.message ?? "Threads like failed");
  }
  return { ok: true };
}
