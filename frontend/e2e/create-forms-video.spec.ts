...
async function selectAccountsAndUploadVideo(
  ...
  if (useManualMedia) {
    await page.getByRole("button", { name: "Click to add video" }).click();
  ...
  await expect(
    page.locator('video[src^="blob:"]').first(),
...
test.describe("Create forms – VideoPostForm", () => {
  ...
  test("Save to Drafts – selects accounts, adds video, fills caption, saves draft, shows success overlay", async ({
    page,
  }) => {
    ...
    await selectAccountsAndUploadVideo(page);
  ...
  test("Post now – selects accounts, adds video, fills caption, clicks Post now, redirects to posts", async ({
    page,
  }) => {
    test.setTimeout(useManualMedia ? 60000 : 45000);
    await loginAsTestUser(page);
    ...
    const uniqueCaption = "Post now video " + Date.now();
    const captionField = page.locator(
      'textarea[placeholder="Add a caption..."]',
    ).first();
  ...
  test("Schedule – selects accounts, adds video, fills caption, enables schedule, sets future time, schedules", async ({
    page,
  }) => {
    test.setTimeout(useManualMedia ? 70000 : 50000);
    await loginAsTestUser(page);
    await page.goto("/dashboard/create/video");
    await page.waitForLoadState("networkidle");
    await expect(page).toHaveURL(/\/dashboard\/create\/video/);
    ...
    const uniqueCaption = "Scheduled video " + Date.now();
    const captionField = page.locator(
      'textarea[placeholder="Add a caption..."]',
    ).first();
    await expect(captionField).toBeVisible({ timeout: 5000 });
