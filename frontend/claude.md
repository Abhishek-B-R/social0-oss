...
- **Video validation (client):** Do not block uploads by aspect ratio; `getAspectRatioGuidance(ratio)` returns `null` for standard ratios (9:16, 16:9, 1:1, 4:5, 2:3, 3:4, 4:3 ~5% tolerance) or one warning for non-standard. Enforce max duration with `MAX_VIDEO_DURATION_SECONDS` (300) and `getVideoDuration(file)` from `lib/video-duration.ts`.
...
| Video (client)    | Aspect: `lib/video-aspect-ratio.ts` — optional non-standard warning only. Duration: `lib/video-duration.ts` (max 300s). |
