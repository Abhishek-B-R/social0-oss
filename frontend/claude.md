...
- **Video validation (client):** Do not block uploads by aspect ratio; show `getAspectRatioGuidance(ratio, { tiktokSelected })` (stronger warning when TikTok is selected and ratio is not ~9:16, 16:9, or 1:1). Enforce max duration with `MAX_VIDEO_DURATION_SECONDS` (300) and `getVideoDuration(file)` from `lib/video-duration.ts`.
...
| Video (client)    | Aspect: `lib/video-aspect-ratio.ts` — guidance only (see `getAspectRatioGuidance`). Duration: `lib/video-duration.ts` (max 300s). |
