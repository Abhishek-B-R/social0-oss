...
- **`app/`** — App Router: routes, layouts, server components, server actions.
  ...
  - **`app/actions/`** — Server actions (`"use server"`): `publish.ts`, `posts.ts`, `repost.ts`, `resurface.ts`.
  - **`app/dashboard/`** — Dashboard layout and all dashboard pages; layout enforces auth and wraps content with sidebar + bottom nav.
- **`components/`** — Reusable React components; `components/ui/` for design system primitives; `components/dashboard/` for dashboard-specific UI.
...
- **Platform IDs:** Use the canonical list and types from `lib/platforms.ts` (`Platform`, `PLATFORMS`, `PLATFORM_OAUTH_CONFIG`). OAuth-only platforms are in `PLATFORM_OAUTH_CONFIG`; BYOK (Bluesky, Dev.to, Hashnode, Medium) are `null` there and have separate connect flows (e.g. `app/api/connect/bluesky/byok/route.ts`).
- **Content types:** Post types (text, image, video, blog, threads, collection) and their allowed platforms are defined in `lib/content-types.ts`; keep platform IDs in sync with `lib/platforms.ts`.
- **Redirects in route handlers:** Use a small helper that accepts a string URL (or a safe fallback) and calls Next.js `redirect()`. In OAuth callbacks we use `safeRedirect(url, fallback)` so we never pass an object or non-URL to `redirect()` (see Known mistakes).
...
- **Schema:** Single source of truth is `db/schema.ts`. Enums: `platform`, `post_status`, `publication_status`. Key tables: `user`, `session`, `account`, `verification` (Better Auth); `connected_accounts`, `media_uploads`, `posts`, `post_publications`, `user_settings`, `resurface_schedules`, `resurface_events`, `auto_plugs`.
...
- **Generic OAuth 2.0:** Entry is `app/api/connect/[platform]/route.ts` (build state, redirect to provider). Callback is `app/api/connect/[platform]/callback/route.ts` (exchange code, store tokens, redirect to dashboard/connections or error).
...
- **Instagram (Meta):** Two paths: (1) Direct Instagram OAuth (`/api/connect/instagram/...`), (2) Instagram via Facebook Page (`/api/connect/instagram-facebook/...` — list Pages, then Pages with linked Instagram).
...
- **Facebook:** Can connect a Page via `/api/connect/facebook` (OAuth then Page list) or via Page select flow and `app/api/connect/facebook/connect-page/route.ts` (POST with `tokenId` + `pageId`).
- **BYOK:** Bluesky, Dev.to, Hashnode, Medium use “bring your own key” routes (e.g. `byok/route.ts`) and no OAuth callback.
...
- `app/api/connect/facebook/connect-page/route.ts`
...
**Fix:** Use a small helper that coerces to a string URL and falls back to a safe path. In OAuth callbacks we use:

- `safeRedirect(url: unknown, fallback: string): never`
- Only redirect to a string that is either a path starting with `/` or an absolute `http`/`https` URL; otherwise use `fallback`.
...
**Fix:** The Facebook OAuth callback saves **all** connected pages in a loop. Each page gets its own row in `connected_accounts` with a unique `platformUserId` (the page ID). The old “select page” UI is gone — do not re-add it.
...
- **Auth:** All dashboard and API routes that touch user data must check session (e.g. `auth.api.getSession({ headers: await headers() })`). Cron routes must be protected by a shared secret (e.g. `CRON_SECRET`) and use constant-time comparison; in development the cron auth check can be skipped for convenience.
...
- **Cron:** Do not expose cron endpoints without auth. Validate `Authorization: Bearer <CRON_SECRET>` with constant-time compare in production.
...
- **Rate limits:** We have optional `platform_rate_limits` table; actual rate limiting is not fully implemented. Prefer not to burst large numbers of requests to a single platform.
- **Cron:** Vercel Cron has limits on frequency and duration. Our crons (publish-scheduled, resurface, autoplug) are daily. Keep cron handlers idempotent and within the configured `maxDuration` (e.g. 60s for publish-scheduled).
...
| Safe redirect URL| Use `safeRedirect(url, fallback)` (or equivalent); never pass object to `redirect()`. |
...
| Cron auth         | Bearer token + constant-time compare; skip only in development if desired. |
