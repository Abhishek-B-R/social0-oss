...
function StaticNavRow({ label, icon: Icon, experimental }: StaticNavItem) {
  return (
    <div className="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium text-sidebar-text">
...
function StaticSection({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <p className="mb-0.5 px-2 text-xs font-semibold uppercase tracking-wide text-sidebar-muted">
        {title}
      </p>
      {items.map((item) => (
...
const DEVELOPER_ITEMS: StaticNavItem[] = [{ label: "Developer", icon: Key }];
      ...
      >
        <div className="flex shrink-0 flex-col gap-2.5 p-3">
          <div className="flex w-full items-center justify-between gap-1.5">
            ...
            <span
              className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-sidebar-muted"
              aria-hidden
            >
          ...
          <div className="flex flex-col gap-1">
            ...
            <SkeletonBone className="h-9 w-full rounded-lg bg-sidebar-active" />
          </div>

          <div
            className={cn(
              "sidebar-create-post-cta flex w-full items-center justify-center gap-1.5 rounded-xl bg-accent px-3 py-2 text-sm font-semibold text-accent-foreground shadow-sm",
            )}
          >
            Create post
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          <nav className="flex flex-col gap-3 p-3 pt-0">
            ...
            <StaticSection title="Developer" items={DEVELOPER_ITEMS} />
          </nav>
        </div>

        <div className="relative z-20 shrink-0 border-t border-sidebar-border bg-sidebar-bg p-2.5">
          <div className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5">
            <SkeletonBone className="h-8 w-8 shrink-0 rounded-full bg-sidebar-active" />
