  ...
  return (
    <div className="rounded-2xl border border-border bg-bg-elevated p-6 shadow-sm">
      <div className="flex flex-wrap items-center gap-2 sm:gap-3 justify-between mb-4">
        <div className="h-6 w-24 rounded bg-bg-muted animate-pulse" />
        <div className="h-9 w-32 rounded-lg bg-bg-muted animate-pulse" />
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        {Array.from({ length: 8 }).map((_, i) => (
          <div
            key={i}
            className="flex items-center gap-3 p-4 rounded-xl border border-border bg-bg-muted/30 animate-pulse"
          >
            <div className="size-10 rounded-full bg-bg-subtle shrink-0" />
