...
export async function POST(req: NextRequest) {
  ...
  const validReturnTo =
    ...
    returnTo.startsWith("/") &&
    !returnTo.startsWith("//");
  const redirectTo = validReturnTo ? returnTo : "/dashboard/connections";
  ...
  if (existing) {
    ...
    const limitCheck = await checkAccountLimits(session.user.id, "instagram");
    if (!limitCheck.allowed) {
      const limitUrl = new URL("/dashboard/connections", req.url);
      limitUrl.searchParams.set("error", "limit_reached");
